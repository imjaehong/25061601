...
static int Check_Collision(void) // 충돌 및 도착 여부 판정
{
	int col = 0;
...
static int Car_Move(void) // 자동차 이동 및 충돌 체크
{
...
static void k0(void) // 조그키 방향별 이동 함수 (UP)
{
	if(frog.y > Y_MIN) frog.y -= FROG_STEP;
}

static void k1(void) // 조그키 방향별 이동 함수 (DOWN)
{
	if(frog.y + frog.h < Y_MAX) frog.y += FROG_STEP;
}

static void k2(void) // 조그키 방향별 이동 함수 (LEFT)
{
	if(frog.x > X_MIN) frog.x -= FROG_STEP;
}

static void k3(void) // 조그키 방향별 이동 함수 (RIGHT)
{
	if(frog.x + frog.w < X_MAX) frog.x += FROG_STEP;
}

static int Frog_Move(int k) // 개구리 이동 (조그키 방향에 따라) 및 충돌 체크
{
...
static void Game_Init(void) // 초기화
{
	score = 0;
	Lcd_Clr_Screen();
...
static void Draw_Object(QUERY_DRAW * obj) // 오브젝트 화면에 그리기
{
